...
import { ContextSanitizer, ContextValidator, ContextTransformer } from './context-utils';
...
export class PageContextDetector {
  ...
  detectFromUrl(url: string): PageContextDetection {
    try {
      ...
      if (url.startsWith('http') || url.startsWith('/')) {
        const { page, section } = ContextTransformer.pathToPageSection(urlObj.pathname);
    ...
    for (const [page, patterns] of Object.entries(pagePatterns)) {
      for (const pattern of patterns) {
        if (pattern.test(lowerMessage)) {
          const confidence = this.calculateChatConfidence(lowerMessage, pattern);
  ...
  detectFromMultipleSources(sources: {
    ...
    const combinedMetadata = detections.reduce((acc, detection) => ({
      ...acc,
      ...detection.metadata
    }), {});
    ...
    return {
      ...bestDetection,
      confidence: Math.min(bestDetection.confidence + (detections.length - 1) * 0.05, 1.0),
      metadata: {
        ...combinedMetadata,
  ...
  trackContext(context: ToolContext, source: string): void {
    const previousContext = this.contextHistory.length > 0 
...
export class ContextTracker {
   ...
   */
  static detectCurrentContext(overrides: Partial<ToolContext> = {}): ToolContext {
