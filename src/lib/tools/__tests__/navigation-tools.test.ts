...
describe('NavigateToPageTool', () => {
  ...
  describe('executeInternal', () => {
    ...
    it('should reject invalid page', async () => {
      const result = await tool.execute({ page: 'invalid' as any }, mockContext);
    ...
    it('should reject invalid section for page', async () => {
      ...
      expect(result.error?.message).toContain('Invalid section "invalid-section" for page "about"');
    });
...
describe('OpenModalTool', () => {
  ...
  describe('executeInternal', () => {
    ...
    it('should open contact modal with pre-filled data', async () => {
      ...
      const result = await tool.execute({ modal: 'contact', data }, mockContext);
    ...
    it('should open project details modal', async () => {
      const result = await tool.execute({ modal: 'project-details' }, mockContext);
    ...
    it('should open project details modal with project ID', async () => {
      ...
      const result = await tool.execute({ modal: 'project-details', data }, mockContext);
    ...
    it('should reject invalid modal type', async () => {
      const result = await tool.execute({ modal: 'invalid' as any }, mockContext);
...
describe('NavigateToSectionTool', () => {
  ...
  describe('executeInternal', () => {
    ...
    it('should reject invalid section for current page', async () => {
      const result = await tool.execute({ section: 'invalid-section' }, mockContext);

      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('INVALID_SECTION');
      expect(result.error?.message).toContain('Section "invalid-section" not found on about page');
      expect(result.error?.suggestions).toContain('Try one of: hero, experience, background');
    });
    ...
    it('should reject invalid section for specified page', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('INVALID_SECTION');
      expect(result.error?.message).toContain('Section "invalid-section" not found on projects page');
    });
