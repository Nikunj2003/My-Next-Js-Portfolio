  ...
  return (
    <header className="sticky top-0 z-50 mt-2 px-6 py-8 sm:mt-8 sm:px-14 md:px-20">
      <div className="mx-auto flex items-center justify-between lg:max-w-7xl">
        ...
        <nav className="hidden md:flex flex-grow items-center justify-between gap-2 rounded-full px-2 py-2 shadow-md ring-1 ring-zinc-200 backdrop-blur-md dark:ring-accent/50">
          <ul className="flex gap-2 text-sm font-medium">
            {routes.map(({ title, href }, index) => {
              const isActive = pathName === href;

              return (
                <li key={index} className="relative transition-transform duration-100 hover:scale-[1.1]">
                  <Link
                    href={href}
                    className={classNames(
                      isActive
                        ? `font-semibold ${textColor} bg-[#56A5A9] rounded-full px-4 py-3 transition-all duration-300 ease-in-out`
      ...
      <MobileMenu routes={routes} openMenu={isModalOpen} setOpenMenu={setIsModalOpen} />
    </header>
  );
}
