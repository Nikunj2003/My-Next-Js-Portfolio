...
import { ToolContext, ToolResult, ToolExecutionConfig, PortfolioTool } from '@/types/tools';
...
export class ToolExecutionMiddleware {
  ...
  async executeWithMiddleware(
    ...
    try {
      ...
      if (!rateLimitResult.allowed) {
        return this.createErrorResult('RATE_LIMIT_EXCEEDED', rateLimitResult.message || 'Rate limit exceeded');
      }

      // 2. Security validation
      const securityResult = await this.validateSecurity(args, context);
      if (!securityResult.valid) {
        return this.createErrorResult('SECURITY_VIOLATION', securityResult.message || 'Security violation');
      }

      // 3. Argument validation
      const validationResult = await this.validateArguments(tool, args);
      if (!validationResult.valid) {
        return this.createErrorResult('INVALID_ARGUMENTS', validationResult.message || 'Invalid arguments', validationResult.suggestions);
      }

      // 4. Execute the tool
      this.log('info', `Executing tool: ${tool.name}`, { args, context: this.sanitizeContext(context) });
      ...
      if (processedResult.success && this.config.logging?.logSuccessfulExecutions) {
   ...
   * Check rate limiting
   */
  private async checkRateLimit(context: ToolContext): Promise<{ allowed: boolean; message?: string }> {
    if (!this.config.rateLimit) {
      return { allowed: true };
    }

    const key = this.config.rateLimit.keyGenerator?.(context) || context.sessionId;
    const now = Date.now();
  ...
  private async validateSecurity(
    ...
    if (this.config.security.maxArgumentSize && argsSize > this.config.security.maxArgumentSize) {
      return { 
        valid: false, 
  ...
  private async validateArguments(
    ...
    try {
      ...
      if (!valid) {
        ...
        const suggestions = this.generateValidationSuggestions(tool.parameters, errors);
   ...
   */
  private async processResult(result: ToolResult, toolName: string): Promise<ToolResult> {
   ...
   */
  private generateValidationSuggestions(schema: JSONSchema7, errors: unknown[]): string[] {
    ...
    for (const error of errors) {
      ...
      switch (err.keyword) {
        case 'required':
          suggestions.push(`Missing required property: ${err.params?.missingProperty}`);
          break;
        case 'type':
          suggestions.push(`Expected ${err.params?.type} but got ${typeof err.data}`);
          break;
        case 'enum':
          suggestions.push(`Value must be one of: ${err.params?.allowedValues?.join(', ')}`);
          break;
        case 'minLength':
          suggestions.push(`Value must be at least ${err.params?.limit} characters long`);
          break;
        case 'maxLength':
          suggestions.push(`Value must be no more than ${err.params?.limit} characters long`);
          break;
        default:
          suggestions.push(`Check the value format for ${err.instancePath || 'the parameter'}`);
      }
    }
