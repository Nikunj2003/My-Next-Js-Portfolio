...
export class ContextualToolSuggestions {
  ...
  getSuggestions(context: ToolContext, userMessage?: string): ToolSuggestion[] {
    ...
    if (userMessage) {
      const intentSuggestions = this.getIntentBasedSuggestions(userMessage, context);
  ...
  filterToolsByContext(criteria: ToolFilterCriteria): PortfolioTool[] {
    ...
    const pageMapping = this.pageToolMappings[page as keyof typeof this.pageToolMappings];
    if (!pageMapping) {
    ...
    if (criteria.intent && criteria.intent !== 'unknown-intent') {
      const intentTools = this.intentToolMappings[criteria.intent as keyof typeof this.intentToolMappings] || [];
  ...
  getSmartRecommendations(
    ...
    const complementaryTools = this.getComplementaryTools(usagePatterns, context);
    suggestions.push(...complementaryTools);

    // Get workflow-based suggestions
    const workflowSuggestions = this.getWorkflowSuggestions(context, recentToolUsage);
   ...
   */
  private getPageBasedSuggestions(page: string, section?: string): ToolSuggestion[] {
    const suggestions: ToolSuggestion[] = [];
    const pageMapping = this.pageToolMappings[page as keyof typeof this.pageToolMappings];
   ...
   */
  private getIntentBasedSuggestions(message: string, context: ToolContext): ToolSuggestion[] {
    const suggestions: ToolSuggestion[] = [];
    ...
    detectedIntents.forEach(({ intent, confidence }) => {
      const intentTools = this.intentToolMappings[intent as keyof typeof this.intentToolMappings] || [];
  ...
  private getThemeBasedSuggestions(context: ToolContext): ToolSuggestion[] {
    ...
    if (themeToggleTool) {
      suggestions.push({
        ...
        relevance: 0.2,
        reason: `Switch to ${context.theme === 'light' ? 'dark' : 'light'} theme`,
   ...
   */
  private detectIntents(message: string): Array<{ intent: string; confidence: number }> {
    ...
    const intentPatterns = {
      ...
      download: [
        /\b(download|get|save|pdf)\b/,
        /\b(resume|cv|document)\b/
      ],
      theme: [
        /\b(theme|dark|light|mode)\b/,
        /\b(switch|toggle|change)\b/
      ],
      projects: [
  ...
  private isToolRelevantForSection(toolName: string, section: string): boolean {
    ...
    const relevantTools = sectionRelevance[section as keyof typeof sectionRelevance] || [];
   ...
   */
  private getPageHelpData(page: string, section?: string): {
    description: string;
    actions: string[];
    questions: string[];
    navigation: string[];
    ...
    const helpData = {
      home: {
        description: "Welcome to Nikunj's portfolio homepage. Get an overview of skills, experience, and featured projects.",
        actions: [
      ...
      about: {
        description: "Learn about Nikunj's professional background, experience, and skills in detail.",
        actions: [
      ...
      projects: {
        description: "Explore Nikunj's portfolio of projects across different technologies and domains.",
        actions: [
      ...
      resume: {
        description: "View and download Nikunj's professional resume with detailed qualifications and experience.",
        actions: [
          "Download PDF resume",
          "View online version",
      ...
      contact: {
        description: "Contact Nikunj for opportunities, collaborations, or inquiries.",
        actions: [
          "Send a message",
  ...
  private getComplementaryTools(
    ...
    usagePatterns.forEach((count, toolName) => {
      const complementary = complementaryRelations[toolName as keyof typeof complementaryRelations] || [];
