...
describe('ContextualToolSuggestions', () => {
  ...
  describe('getSuggestions', () => {
    ...
    it('should include intent-based suggestions when message provided', () => {
      const result = suggestions.getSuggestions(mockContext, 'show me your projects');
    ...
    it('should boost relevance for section-specific tools', () => {
      ...
      const experienceSuggestion = result.find(s => s.tool.name === 'GetExperience');
    ...
    it('should limit suggestions to 10 items', () => {
      const result = suggestions.getSuggestions(mockContext, 'tell me everything about projects experience skills');
  ...
  describe('getContextualHelp', () => {
    ...
    it('should return help for about page', () => {
      ...
      expect(result.suggestedQuestions.some(q => q.includes('experience'))).toBe(true);
    });
    ...
    it('should return help for projects page', () => {
      ...
      expect(result.availableActions).toContain('Filter projects by technology');
      expect(result.suggestedQuestions.some(q => q.includes('projects'))).toBe(true);
    });
    ...
    it('should return help for resume page', () => {
      ...
      expect(result.suggestedQuestions.some(q => q.includes('download'))).toBe(true);
    });
    ...
    it('should return help for contact page', () => {
      ...
      expect(result.suggestedQuestions.some(q => q.includes('contact'))).toBe(true);
    });
  ...
  describe('getSmartRecommendations', () => {
    it('should recommend complementary tools based on usage', () => {
      ...
      const result = suggestions.getSmartRecommendations(mockContext, recentUsage);
    ...
    it('should increase relevance for frequently used tools', () => {
      ...
      const complementaryTool = result.find(s => s.reason.includes('GetProjects'));
  ...
  describe('intent detection', () => {
    it('should detect navigation intent', () => {
      const result = suggestions.getSuggestions(mockContext, 'go to projects page');
      
      expect(result.some(s => s.context?.includes('intent:navigation'))).toBe(true);
      expect(result.some(s => s.tool.name === 'NavigateToPage')).toBe(true);
    });

    it('should detect information intent', () => {
      const result = suggestions.getSuggestions(mockContext, 'tell me about your experience');
      
      expect(result.some(s => s.context?.includes('intent:information'))).toBe(true);
      expect(result.some(s => s.tool.name === 'GetExperience')).toBe(true);
    });

    it('should detect download intent', () => {
      const result = suggestions.getSuggestions(mockContext, 'download your resume');
      
      expect(result.some(s => s.context?.includes('intent:download'))).toBe(true);
      expect(result.some(s => s.tool.name === 'TriggerDownload')).toBe(true);
    });

    it('should detect theme intent', () => {
      const result = suggestions.getSuggestions(mockContext, 'switch to dark mode');
      
      expect(result.some(s => s.context?.includes('intent:theme'))).toBe(true);
      expect(result.some(s => s.tool.name === 'ToggleTheme')).toBe(true);
    });

    it('should detect contact intent', () => {
      const result = suggestions.getSuggestions(mockContext, 'how can I contact you');
      
      expect(result.some(s => s.context?.includes('intent:contact'))).toBe(true);
      expect(result.some(s => s.tool.name === 'OpenModal')).toBe(true);
    });

    it('should handle multiple intents in one message', () => {
      const result = suggestions.getSuggestions(mockContext, 'show me projects and download resume');
  ...
  describe('priority assignment', () => {
    ...
    it('should assign low priority to tertiary tools', () => {
      ...
      const tertiaryTool = result.find(s => s.tool.name === 'TriggerDownload');
      expect(tertiaryTool?.priority).toBe('low');
    });

    it('should assign priority based on intent confidence', () => {
      const result = suggestions.getSuggestions(mockContext, 'definitely download the resume now');
      
      const downloadTool = result.find(s => s.tool.name === 'TriggerDownload');
  ...
  describe('deduplication', () => {
    it('should remove duplicate tool suggestions', () => {
      const result = suggestions.getSuggestions(mockContext, 'projects and portfolio work');
    ...
    it('should keep highest relevance when deduplicating', () => {
      const result = suggestions.getSuggestions(mockContext, 'show me all your projects');
